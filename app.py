...
# Criar tabela de vigilantes
    cursor.execute('''CREATE TABLE IF NOT EXISTS vigilantes (
...
@app.route('/cadastro_vigilante', methods=['GET', 'POST'])
def cadastro_vigilante():
    if request.method == 'POST':
        ...
        try:
            # Inserir o vigilante no banco de dados
            cursor.execute("INSERT INTO vigilantes (nome) VALUES (?)", (nome,))
            conn.commit()
            return redirect(url_for('cadastro_vigilante'))  # Redireciona após cadastro

        except sqlite3.IntegrityError:
            return "Vigilante já cadastrado."

        finally:
            conn.close()

    return render_template('cadastro_vigilante.html')  # Página de cadastro de vigilante
...
def inserir():
    ...
    # Carregar os vigilantes cadastrados
    conn = sqlite3.connect('controle_portaria.db')
    cursor = conn.cursor()
    cursor.execute("SELECT id, nome FROM vigilantes")
    vigilantes = cursor.fetchall()  # Lista de tuplas (id, nome)
    conn.close()
    ...
    if request.method == 'POST':
        ...
        vigilante_entrada = request.form['vigilante_entrada']  # Agora é o id do vigilante selecionado
        ...
        return render_template('index.html', sucesso="Registro incluído com sucesso!", vigilantes=vigilantes)

    # Caso seja um GET, carrega os dados do registro para o template
    return render_template('form_inserir.html', vigilantes=vigilantes)
...
# Alterando o template para carregar vigilantes
@app.route('/form_inserir', methods=['GET'])
def form_inserir():
    # Carregar vigilantes cadastrados
    conn = sqlite3.connect('controle_portaria.db')
    cursor = conn.cursor()
    cursor.execute("SELECT id, nome FROM vigilantes")
